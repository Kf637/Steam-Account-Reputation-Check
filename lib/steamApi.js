...
(async function(){
  ...
  async function ensureSteamApiKey() { return ''; }

  async function fetchSteamAccountInfo(steamId64) {
    try {
      const resp = await fetch(`/api/steam-account?steamid=${encodeURIComponent(steamId64)}`);
      if (resp.status === 429) {
        // propagate rate limit info
        let retry = null;
        try { const j = await resp.json(); retry = j && j.retry_after; } catch (e) {}
