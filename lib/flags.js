...
(function(){
  // Force image-based flags to ensure consistent rendering on desktop and in canvas exports
  const FORCE_IMAGE_FLAGS = true;
  ...
  if (!FORCE_IMAGE_FLAGS && supportsEmojiFlags(code)) {
      const emoji = countryCodeToFlag(code);
      const px = Math.max(12, Number(size) || 16);
      return `<span class="flag-emoji" data-cc="${code}" style="font-size:${px}px;">${emoji}</span>`;
    }
  ...
  return `<img class="flag-img" data-cc="${code}" alt="${code} flag" src="https://flagcdn.com/${lower}.svg" width="${px}" height="${h}" loading="lazy" referrerpolicy="no-referrer" crossorigin="anonymous" />`;
  };
