...
(function(){
  ...
  const banDays = (typeof ban.DaysSinceLastBan === 'number') ? ban.DaysSinceLastBan : null;
  ...
  if (game > 0) score -= gameWeight * game;
    ...
    // Age
    if (ageDays !== null) {
      if (ageDays < 2) score -= 100; else if (ageDays < 3) score -= 90; else if (ageDays < 14) score -= 65; else if (ageDays < 40) score -= 45; else if (ageDays < 90) score -= 25; else score += 5;
    }
    // Privacy
